...
function renderResult(){
  ...
  <div class="data-note">本测试会保存匿名结果摘要用于内测统计，不收集姓名、手机号、微信号或地址等身份信息。</div>
  ...
  <div class="result-card">
    ...
    ${renderShareStatus()}
    <div id="submissionStatus" class="submission-status">${submissionStatusText()}</div>
  ...
  layout(main);
  submitAnonymousResultOnce(r);
}
function submissionStatusText(){
  if(state.submissionStatus==='saving') return '匿名摘要正在保存。';
  if(state.submissionStatus==='saved') return '匿名摘要已保存，用于内测统计。';
  if(state.submissionStatus==='failed') return '匿名摘要暂时未保存，不影响本次结果。';
  return '匿名摘要会在结果页生成后提交一次。';
}
function updateSubmissionStatusText(){
  const el=document.getElementById('submissionStatus');
  if(el) el.textContent=submissionStatusText();
}
function cleanAnswerMap(map){
  const out={};
  for(const [key,value] of Object.entries(map||{})){
    const n=Number(value);
    if(Number.isInteger(n) && n>=1 && n<=7) out[key]=n;
  }
  return out;
}
function buildAnonymousSubmission(result){
  return {
    source:'web',
    version:'6.9.7-data',
    inputMbti:result.inputMbti,
    currentMbti:result.currentMbti,
    resultMbti:result.initMbti,
    resultObject:result.object.name,
    deletionTitles:result.deletion.slice(0,5).map(item=>item.title),
    retainedTitles:result.retained.slice(0,4).map(item=>item.title),
    confidence:result.confidence,
    answers:{
      current:cleanAnswerMap(state.currentAnswers),
      ref:cleanAnswerMap(state.refAnswers)
    },
    client:{
      device:(typeof window !== 'undefined' && window.matchMedia && window.matchMedia('(max-width: 760px)').matches) ? 'mobile' : 'desktop',
      language:(navigator.language || '').slice(0,20)
    }
  };
}
async function submitAnonymousResultOnce(result){
  ...
  try{
    const res=await fetch('/.netlify/functions/submit-result',{
      ...
      body:JSON.stringify(buildAnonymousSubmission(result))
    });
    ...
    console.warn('Anonymous result save failed', err);
