...
function renderSide(){
  const steps=['现在的你','当前确认','删除日志','初始化报告','剧本保留'];
...
function renderFragmentDone(){
  ...
  const main=`<div class="transition"><div class="panel"><h2>${frag.doneTitle}</h2><p>${frag.doneDesc}</p><div class="sysline">delete.log_${state.fragmentIndex+1} = success // old feedback volume reduced</div><div class="actions">${doneLast?`<button class="btn" onclick="setScreen('loading')">生成初始化设定</button>`:`<button class="btn" onclick="state.fragmentIndex++; setScreen('fragmentIntro')">进入下一段</button>`}</div></div></div>`;
  layout(main);
}
function renderLoading(){
  computeResult();
  const main=`<div class="transition"><div class="panel"><h2>日志清理完成，时间正在回溯。</h2><p>系统正在清空你的人生日志……不是把过去抹掉，而是暂时把那些旧反馈调低音量。接下来，我们会根据你留下来的倾向，生成你的初始化设定。</p><div class="sysline">clear(life_logs); extract(core_tendency); boot(initial_self);</div><div class="actions"><button class="btn" onclick="setScreen('result')">查看报告</button></div></div></div>`;
  layout(main);
}
function renderResult(){
  const r=state.result || computeResult();
  const desc = state.mode==='warm'?r.object.warm:r.object.crazy;
  ...
  <div class="result-card"><div class="result-title">你想怎么看自己？</div><div class="mode-toggle"><button class="mode-btn ${state.mode==='warm'?'active':''}" onclick="state.mode='warm';render()">温柔地看</button><button class="mode-btn crazy ${state.mode==='crazy'?'active':''}" onclick="state.mode='crazy';render()">劳资爱怎么看怎么看</button></div><p class="description">${desc}</p><p class="explain">系统把你回答里最强的旧反馈先调低音量，再根据剩下的倾向生成这个初始化设定。它不是心理诊断，也不是最终答案，只是一个反事实推演版本：如果少一些被迫适应，你可能更接近什么样子。</p></div>
  <div class="result-card"><div class="result-title">MBTI 反推比例</div><div class="list-grid">${axisPercentHTML(r.initScores)}</div></div>
  <div class="result-card"><div class="result-title">平行人生剧本（保留功能，暂不重点优化）</div><div class="script-grid">${DATA.scripts.map(s=>`<div class="script-card ${state.scriptId===s.id?'active':''}" onclick="state.scriptId='${s.id}';render()"><h4>${s.name}</h4><p>${s.desc}</p></div>`).join('')}</div>${state.scriptId?`<p class="description">${DATA.scripts.find(s=>s.id===state.scriptId).story}</p>`:''}</div>
  <div class="result-card"><button class="btn secondary" onclick="toggleDeep()">展开深层分析</button><div id="deep" class="deep"><pre class="debug-table">${escapeHtml(JSON.stringify({inputMbti:r.inputMbti,currentMbti:r.currentMbti,initMbti:r.initMbti,object:r.object.name,deletion:r.deletion,retained:r.retained,traitScores:r.traitScores,times:state.times},null,2))}</pre></div><div class="footer-actions"><button class="btn" onclick="downloadShare()">生成分享图</button><button class="btn secondary" onclick="exportJSON()">导出匿名 JSON</button><button class="btn ghost" onclick="copyResult()">复制结果文本</button><button class="btn ghost" onclick="location.reload()">重新开始</button></div><textarea id="copyText" class="textarea-copy" readonly>${makeShareText(r, modeTitle, desc)}</textarea></div>`;
  layout(main);
}
