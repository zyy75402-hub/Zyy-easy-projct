...
const state = { screen: 'home', mbti: 'INFP', confidence: 'B', answers: {} };
...
function renderHome() {
  app.innerHTML = `<section class="hero">
    <div class="kicker">BBTI 7.0.2 · 小动物结果版</div>
    <h1>BBTI<br>底色原型回推</h1>
    <p class="lead">本版本使用 33 道题完成底色回推，并将唯一结果映射为 BBTI 小动物。页面不会上传数据，结果页会优先展示可截图的小动物身份卡。</p>
    <div class="actions"><button class="primary" data-action="start">开始 33 题测试</button></div>
    <div class="notice">提示：结果页只展示一个小动物身份；复杂模型信号仅保留在默认折叠的调试区。</div>
  </section>`;
}
function renderSetup() {
  app.innerHTML = `<div class="topbar"><div class="progress-wrap"><div class="progress" style="width: 3%"></div></div></div>
  <section class="panel">
    <div class="kicker">Step 01 · 当前输入</div>
    <h2>先给系统一个“现在的你”</h2>
...
function renderQuestions() {
  ...
  <section class="panel">
    ...
    <h2>按第一反应选择更接近的一项</h2>
    <p class="muted">题目没有好坏，也不是心理诊断。它会用于生成你的唯一 BBTI 小动物结果。</p>
  </section>
...
function renderResult() {
  ...
  const cardName = profile ? profile.cardName : main.name;
  ...
  app.innerHTML = `<section class="result-head animal-page">
    ...
    <div class="share-card" aria-label="可截图的小动物结果卡">
      ...
      <div class="animal-subtitle">${esc(cardName)}</div>
      <div class="animal-species">${esc(animal)}</div>
      <div class="animal-slogan">${esc(slogan)}</div>
    </div>
    <div class="actions"><button class="secondary" data-action="questions">修改答案</button><button class="ghost" data-action="home">回到首页</button></div>
  </section>
  <section class="result-grid animal-detail-grid">
    ...
    <div class="result-card"><h3>当前输入</h3><p>用户输入 MBTI：<b>${state.mbti}</b></p><p>MBTI 置信度：<b>${confidenceItem().text}</b></p><p class="muted">系统将 MBTI 视为当前人格自我描述，并结合 33 道题进行回推。</p></div>
    <div class="result-card"><h3>系统校准后的当前外壳</h3>${r.shell.length ? `<ul>${r.shell.map((item) => `<li>${item}</li>`).join('')}</ul>` : '<p class="muted">当前外壳信号不强。</p>'}<p class="muted">这部分不等于底色，更像长大后形成的外层反应。</p></div>
  </section>
